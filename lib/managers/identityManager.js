...
class IdentityManager extends RequestManager {
  getPolls(pollIds, callback) {
    ...
    var requestUrl = this.routes.url('is:getDiscussions', {}, {
      query: {ids: _.uniq(pollIds).join(',')}
    });
  ...
  getCategories(callback) {
    var requestUrl = this.routes.url('is:getCategories');
    return this.makeRequest(internals.apiUri, {method: 'get'}, function (err, resp, body) {
      if (err) {
        return callback(err);
      }
  ...
  getPollsFromCategory(categoryId, callback) {
    var requestUrl = this.routes.url('is:getCategoryPolls', {categoryId: categoryId}, {
      query: {limit: 50}
    });
