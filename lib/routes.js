...
class Routes {
  ...
  generateRoutes(callback) {
    ...
    this.routes.createForAccount = {href: this.serviceApiRoots.ps + '/accounts/{accountId}/photos'};
  ...
  injectApiRoot() {
    ...
    this.routesConfig.splice(0, 0, {method: 'GET', path: '/', handler: function (request, reply) {
      return reply({
        ...
        resources: _.chain(_this.routes).map(function (value, key) {
          if (key.indexOf(_this.namespace) !== 0) {
            return null;
          }
